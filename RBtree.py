...
class RBTree(object):
  ...
  def _insertFixedUpRBT(self,current):
    while current.parent.color == RED: 
      ...
      if current.parent == current.parent.parent.left:
        # precedessor is grandfather left-child. so uncle would be right-child of grandfather
  ...
  def _deleteFixedUpRBT(self,current):
    ...
    while current != root and current.color != RED:
      ...
      if current == current.parent.left:
        ...
        if sibling.right.color == BLACK and sibling.left.color == BLACK:
          ...
          if sibling.right.color == BLACK:
            # case 3) sibling right child is black, and left child is red
            sibling.color = RED
  ...
  def delete(self,val):
    ...
    delegator = delete if delete.left == NIL or delete.right == NIL else self.successor(delete)
    ...
    # because we need to determine child located on left side or right side of a deleted node while fixing up
    ...
    if delegator != delete:
      # if the deleted node is delegator, we need to dupliate all attributes of the deleted node into a delegator
      delegator.dup(delete)


    if delegator.color == BLACK:
      # if the deleted node is black, we must fix its child to meet the Theorems of RB tree.
      self._deleteFixedUpRBT(child)

  def t_height(self, node):
    return 0 if node == NIL else 1+max(self.t_height(node.left), self.t_height(node.right))
